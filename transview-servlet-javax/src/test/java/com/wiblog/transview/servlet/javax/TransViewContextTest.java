...
public class TransViewContextTest {

    private static final byte[] PNG_SIGNATURE = new byte[]{(byte) 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A};

    @Test
    public void previewFileSetsCacheHeadersAndContentLength() throws Exception {
    ...
    @Test
    public void previewFileReturns304WhenEtagMatches() throws Exception {
        File file = File.createTempFile("transview-plain-", ".txt");
        try {
            Files.write(file.toPath(), "hello".getBytes(StandardCharsets.UTF_8));
    ...
    @Test
    public void previewFileReturns416ForInvalidRange() throws Exception {
        File file = File.createTempFile("transview-plain-", ".txt");
        try {
            Files.write(file.toPath(), "hello".getBytes(StandardCharsets.UTF_8));
            MockHttpServletRequest request = new MockHttpServletRequest();
    ...
    @Test
    public void convertInputStreamWritesToOutputStream() {
    ...
    @Test
    public void previewOutputStreamDetectsActualContentTypeBeforeWriting() throws Exception {
        MockHttpServletResponse response = new MockHttpServletResponse();
    ...
    private static OutputStream createPreviewOutputStream(MockHttpServletResponse response) throws Exception {
        Method method = TransViewContext.class.getDeclaredMethod("previewOutputStream", javax.servlet.http.HttpServletResponse.class);
